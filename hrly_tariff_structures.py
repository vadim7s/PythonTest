...
reg_qry = "select Contract,State,left(ContractPOD,10) as NMI,ContractStartDate," \
            ...
            #"where AGL_PRICING.dbo.CONTRACT_ATTRIBUTE.Contract = '9408807678'"
...
elec_registers = elec_registers.groupby(['Contract','NMI','Suffix'], as_index=False).agg({'State': lambda x: x.iloc[0], 'ContractStartDate': lambda x: x.iloc[0],
...
elec_registers.to_csv("elec_registers.csv",header=True, index=False)
...
ret_structure = pd.merge(ret_structure,elec_registers.drop(columns=['LogicalRegisterNumber','RateType','State','RetailTariffType','NetworkTariff','NetworkTariffType']),
                            how='inner',left_on='RetailTariff',right_on='RetailTariff')
nwk_structure = pd.merge(nwk_structure,elec_registers.drop(columns=['LogicalRegisterNumber','RateType','State','RetailTariff','RetailTariffType']),
...
#first drop some columns from network
nwk_structure.drop(columns=['state','Class','Patch','ContractStartDate','ContractEndDate'], inplace=True)
nwk_structure.to_csv("D:/Temp/nwk_structure.csv",mode="a",header=False,index=False)

# creating a empty bucket to save result
df_result = pd.DataFrame(columns=(ret_structure.columns.append(nwk_structure.columns)).unique())
df_result.to_csv("D:/Temp/combined_structure.csv",index_label=False)

# deleting  to save memory
del(nwk_structure)

def chunk_data(x):
    df2=pd.merge(ret_structure,x, how='inner',on=['Contract','NMI','Suffix','date','hour','component_group'])
    df2.to_csv("D:/Temp/combined_structure.csv",mode="a",header=False,index=False)
    
reader = pd.read_csv("D:/Temp/nwk_structure.csv", chunksize=1000000) # chunksize depends with you colsize
[chunk_data(r) for r in reader]


''' OLD code
combined_structure = pd.merge(ret_structure,nwk_structure,how='outer',
                                on=['Contract','NMI','Suffix','date','hour','component_group'],
                                indicator=True)
...
print(retail_orphans_usg_CL.head(3))
print(network_orphans_usg_CL.head(3))
'''
