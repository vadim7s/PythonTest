...
reg_qry = "select Contract,State,left(ContractPOD,10) as NMI,ContractStartDate," \
            ...
            #"where AGL_PRICING.dbo.CONTRACT_ATTRIBUTE.Patch = 'UNITED'"
...
ret_structure.to_csv('D:/Temp/retail_structure.csv',header=True, index=False)
nwk_structure.to_csv('D:/Temp/network_structure.csv',header=True, index=False)
elec_registers.to_csv('D:/Temp/elec_registers.csv',header=True, index=False)
...
combined_structure = pd.merge(ret_structure,nwk_structure.drop(columns=['state','Class','Patch','ContractStartDate','ContractEndDate']),
