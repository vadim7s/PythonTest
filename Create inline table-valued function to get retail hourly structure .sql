/****** create function to return hourly structure for a tariff fed as a parameter  ******/
/****** but could not run as no access to create functions in AGL_MDS               ******/

CREATE FUNCTION RetailStructureHR (@tariff varchar)
RETURNS TABLE
AS
RETURN
(
...
and x.retail_tariff=@tariff
--option (maxrecursion 365)  ->this may hep for max recursion issue
);
GO
